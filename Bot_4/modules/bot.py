...
def create_found(result_search_data, skip):
    ...
    for i in search:
        ...
        if (i.get('city') is not None and i.get('home_town') is not None and
            ...
            photos = get_photo(
                ...
                count=10)
            # print(f'skip  = {skip}')
            # Если фото присутствуют в количестве 3х и не находятся среди уже просмотренных:
            if len(photos) == 3 and i['id'] not in skip:
                found.append({
                    'id': i['id'],
...
def view(user_id, answer, skip):
    ...
    return {'favorites': favorites, 'black': black, 'viewed': viewed}
